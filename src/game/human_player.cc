// #include "include/human_player.hpp"

// namespace Puzznic
// {
//   HumanPlayer::HumanPlayer(std::string level) { activeBoard_ = std::make_shared<BoardState>(level); }
...
//   void HumanPlayer::MakeMove(const Move& move) { activeBoard_->MakeMove(move); }

//   void HumanPlayer::GenerateAvailableMoves(Coord_t item)
//   {
...
//   bool HumanPlayer::ItemInItemList(Coord_t item)
//   {
...
//   const std::map<Coord_t, int> HumanPlayer::GetRenderItems()
//   {
//     const std::map<int, Item_t> ItemIds = activeBoard_->GetItemMap();
//     std::map<Coord_t, int> ItemCoordsMap;
